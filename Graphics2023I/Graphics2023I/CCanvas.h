...
public:
	...
	enum AddressMode{ADDRESS_MODE_BORDER, ADDRESS_MODE_WRAP, ADDRESS_MODE_MIRROR,
	ADDRESS_MODE_CLAMP};
	struct VERTEX
	{
		VECTOR4D Position;
...
public:
	...
	static CCanvas* CreateCanvasFromFile(const char* pszFileName, PIXEL (*Alpha)(PIXEL) = nullptr); //Load from disk
	void InverseTextureMapping(
		const VERTEX V[], CCanvas* pTexture,
		PIXEL(CCanvas::* sampler)(float, float));
	void Attach(CCanvas* pTarget);

	void DrawPointList(VERTEX* pVertex, int nVertex);
	void DrawLineStrip(VERTEX* pVertex, int nVertex);
	void DrawLineList(VERTEX* pVertex, int nVertex);
	void DrawTriangleList(VERTEX* pVertex, int nVertex);
	void DrawTriangleListIndexed(VERTEX* pVertex, long* pIndex, int nIndexes);
	void DrawTriangleStripIndexed(VERTEX* pVertex, long* pIndex, int nIndexes);
};