...
CCanvas::PIXEL CCanvas::AlphaBlendSrc(PIXEL dst, PIXEL src)
{
	//    Opacity : 1 max
	//              0 min
	//   R = dst * (1-src.a) + src*src.a
	//     = dst - dst*src.a + src*src.a
	//     = dst + (src - dst)*src.a
	if (src.a == 255)
		return src;
	if (src.a == 0)
		return dst;
	return { (unsigned char)((int)dst.r + ((((int)src.r - (int)dst.r) * src.a) / 256)),
			 (unsigned char)((int)dst.g + ((((int)src.g - (int)dst.g) * src.a) / 256)),
			 (unsigned char)((int)dst.b + ((((int)src.b - (int)dst.b) * src.a) / 256)),
			 (unsigned char)((int)dst.a + ((((int)src.a - (int)dst.a) * src.a) / 256)) };
}
...
void CCanvas::InverseMapping(MATRIX4D M, CCanvas* pSource, PIXEL(CCanvas::* sampler)(float, float))
{
	...
	for (int y = 0; y < m_nSizeY; y++)
	{
		...
		if (m_pLimits[y].left < m_pLimits[y].right)
		{
			...
			while (c--)
			{
				*pPixel++ = AlphaBlendSrc(*pPixel,(pSource->*sampler)(__x, __y));
				__x += M.m00;
				__y += M.m01;
			}
		}
...
CCanvas* CCanvas::CreateCanvasFromFile(const char* pszFileName, PIXEL(*Alpha)(PIXEL))
{
	CCanvas* pNewCanvas;
	...
	case 32:
		for (int j = pNewCanvas->m_nSizeY - 1; j >= 0; j--)
		{
			for (int i = 0; i < pNewCanvas->m_nSizeX; i++)
			{
				...
				file.write((char*)&p.a, sizeof(p.a));
				if (Alpha)
					(*pNewCanvas)(i, j) = Alpha((*pNewCanvas)(i, j));
			}
		}
		break;
	case 24:
		for (int j = pNewCanvas->m_nSizeY - 1; j >= 0; j--)
		{
			...
			for (int i = 0; i < pNewCanvas->m_nSizeX; i++)
			{
				...
				(*pNewCanvas)(i, j) = Alpha ? Alpha(P) : P;
			}
		}
		break;
	case 8:
		...
		for (int j = pNewCanvas->m_nSizeY - 1; j >= 0; j--)
		{
			...
			for (int i = 0; i < pNewCanvas->m_nSizeX; i++)
			{
				...
				PIXEL P = { Color.rgbRed, Color.rgbGreen, Color.rgbBlue, 255 };
				(*pNewCanvas)(i, j) = Alpha ? Alpha(P) : P;
			}
		}
		break;
...
void CCanvas::Attach(CCanvas* pTarget)
{
	pTarget->m_pBuffer = m_pBuffer;
	pTarget->m_nPitch = m_nPitch;
	pTarget->m_nSizeX = m_nSizeX;
	pTarget->m_nSizeY = m_nSizeY;
	pTarget->m_pLimits = m_pLimits;
	pTarget->m_am = m_am;
	pTarget->m_Border = m_Border;

}

void CCanvas::DrawPointList(CCanvas::VERTEX* pVertex, int nVertex)
{
	for (int i = 0; i < nVertex; i++)
	{
		(*this)(pVertex->Position.x, pVertex->Position.y) = pVertex -> Color;
		pVertex++;
	}
}

void CCanvas::DrawLineStrip(CCanvas::VERTEX* pVertex, int nVertex)
{
	int x0, y0, x1, y1;
	x0 = pVertex->Position.x;
	y0  = pVertex->Position.y;
	nVertex--;
	pVertex++;
	while (nVertex--)
	{
		x1 = pVertex->Position.x;
		y1 = pVertex->Position.y;
		Line(x0, y0, x1, y1, pVertex->Color);
		x0 = x1;
		y0 = y1;
		pVertex++;
	}
...
void CCanvas::DrawLineList(CCanvas::VERTEX* pVertex, int nVertex)
{
	int x0, y0, x1, y1;
	while (nVertex)
	{
...
void CCanvas::DrawTriangleList(CCanvas::VERTEX* pVertex, int nVertex)
{
	while (nVertex > 2)
	{
		...
		Line(x0, y0, x1, y1, pVertex->Color);
		pVertex++;
		x2 = pVertex->Position.x;
		y2 = pVertex->Position.y;
		Line(x1, y1, x2, y2, pVertex->Color);
		pVertex++;
		Line(x2, y2, x0, y0, pVertex[-3].Color);
		nVertex -= 3;
	}
}

void CCanvas::DrawTriangleListIndexed(CCanvas::VERTEX* pVertex, 
	long* pIndex, int nIndexes)
{
	VERTEX* pV0, * pV1, * pV2;
	while (nIndexes)
	{
		pV0 = pVertex + pIndex[0];
		pV1 = pVertex + pIndex[1];
		pV2 = pVertex + pIndex[2];
		nIndexes -= 3;
		pIndex += 3;
		Line(pV0->Position.x, pV0->Position.y, pV1->Position.x, pV1->Position.y,
			pV2->Color);
		Line(pV1->Position.x, pV1->Position.y, pV2->Position.x, pV2->Position.y,
			pV2->Color);
		Line(pV2->Position.x, pV2->Position.y, pV0->Position.x, pV0->Position.y,
			pV2->Color);
	}
}

void CCanvas::DrawTriangleStripIndexed(CCanvas::VERTEX* pVertex,
	long* pIndex, int nIndexes)
{
	VERTEX* pV0, * pV1, * pV2;
	pV0 = pVertex + pIndex[0];
